...
It quotes calibrated probabilities with honest uncertainty and keeps a running
scorecard in `picks.xlsx` (predicted growth % AND realized growth % per pick).
It never buys anything.

The signal engine is **v3** (`config.ENGINE`); every scan, calibration and
recorded pick is engine-stamped. See `SCOUT-DESIGN.md` for lineage.
...
Deadline) and MISSes, rebuilds the Track Record sheet. Note any newly
...
1. **Next earnings date.** If it falls before `horizon_end`, that's a binary
   event inside the window → downgrade the grade one notch (A→B→C) and say so
   in the thesis.
...
2. TWO tables in plain words. "Best Overall (top 3)": Stock | Price | Chance
   of +5% | Usual peak gain | Days it usually takes | Chance of a -5% dip
   first | Confidence | one-line why. Then "Biggest Gain (top 3, only 62%+
   chance)": same columns ordered by gain. Explain once, simply: the overall
   list balances chance × gain size × speed × safety; the gain list chases
   the biggest usual peaks among stocks still clearing the 62% chance bar.
...
4. The caveats from `last_scan.json`, briefly, plus: research scorecard, not
   financial advice; nothing was bought.
