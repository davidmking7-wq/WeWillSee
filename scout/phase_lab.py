...
def book_return(c, scan, n, offset):
    """Equal-weight top-n return `offset` trading days after entry."""
    pos = scan["pos"]
    basis = c.iloc[pos]
    legs = []
    for sym in scan["ranked"][:n]:
        if sym not in c.columns or pd.isna(basis.get(sym)):
            continue
        seg = c[sym].iloc[pos + 1: pos + 1 + offset].dropna()
        if len(seg) >= min(5, offset):
            legs.append(float(seg.values[-1] / basis[sym] - 1))
    return float(np.mean(legs)) if legs else 0.0


def sleeve_curve(c, by_pos, grid, n, offset, step):
    ...
    curve, cum, skipped = {}, 1.0, 0
    entry, scan = None, None
    for t in grid:
        if (t - grid[0] - offset) % H == 0 and t >= grid[0] + offset:
            if entry is not None and scan is not None:
                cum *= (1 + book_return(c, scan, n, H))
            entry, scan = t, by_pos.get(t)
            if scan is None:
                skipped += 1
        if entry is None or scan is None:
            curve[t] = cum
        else:
            off = t - entry
            curve[t] = cum if off == 0 else cum * (1 + book_return(
                c, scan, n, off))
...
def sweep(args, per):
    ...
    for n in [int(x) for x in args.sweep.split(",")]:
        curves = {p: sleeve_curve(c, by_pos, grid, n, p * args.step,
                                  args.step)[0] for p in range(per)}
        avgs, comps = [], []
        for p in range(per):
            m = portfolio_lab.metrics(sampled(curves[p], grid, per))
            avgs.append(m["avg"])
            comps.append(m["compounded"])
        ladder = {t: float(np.mean([curves[p][t] for p in range(per)]))
                  for t in grid}
        ml = portfolio_lab.metrics(sampled(ladder, grid, per))
        ...
        print(f"top{n:<5}{' '.join(f'{a:>6.2f}' for a in avgs):<46}"
              ...
              f"{ml['pct_ge5']:>7}")
    spy = {t: float(c["SPY"].iloc[t]) for t in grid}
    b = spy[grid[0]]
    m = portfolio_lab.metrics(sampled({k: v / b for k, v in spy.items()},
                                      grid, per))
    out["SPY"] = m
...
def main() -> None:
    ...
    curves, skips = {}, {}
    for p in range(per):
        curves[p], skips[p] = sleeve_curve(c, by_pos, grid, args.n,
                                           p * args.step, args.step)
    ladder = {t: float(np.mean([curves[p][t] for p in range(per)]))
              for t in grid}
    ...
    spy = {t: float(c["SPY"].iloc[t]) for t in grid}
    b = spy[grid[0]]
    m_spy = portfolio_lab.metrics(sampled({k: v / b for k, v in spy.items()},
                                          grid, per))
    ...
    for tag, sl in (("FIRST HALF", slice(0, half + 1)),
        ...
        print(HDR)
        for p in range(per):
            m = portfolio_lab.metrics(sampled(curves[p], ks, per))
            if m:
                out["strategies"][f"phase{p}"][tag.lower().replace(" ", "_")] = m
                print(line(f"phase {p} (entry +{p*args.step}td)", m))
        m = portfolio_lab.metrics(sampled(ladder, ks, per))
        if m:
