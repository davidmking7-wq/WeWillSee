...
### H31 — portfolio CONSTRUCTION, not selection (scout/construction_lab.py)

Registered 2026-08-09 BEFORE the first number, from Round 4's decomposition:
the equal-weight gated pool is -0.216 Sharpe against cap-weighted SPY before a
stock is picked, while the whole selection layer adds +0.030 back. Mechanism:
cap-weighting rides winners — a name that doubles doubles its own weight — a
momentum tilt at zero turnover and zero cost, while equal-weighting trims
winners, buys losers, pays the turnover, and carries the volatility of its
smallest constituents.

CONFIRMATORY regime (strong external prior — the SPY/RSP divergence is public
record; pre-registered; one specification per cell). 53 variants per universe,
106 in total; registry N rises by **106**.

| # | date | hypothesis (mechanism, one sentence) | expected sign | status |
|---|---|---|---|---|
| H31a | 2026-08-09 | DECOMPOSITION. On IDENTICAL holdings and identical windows, cap-weighting beats equal-weighting on Sharpe — the construction cost, isolated from selection. | Sharpe(cap) - Sharpe(equal) > 0 | **CONFIRMED in sign, in all six pairs, and NOT significant.** PIT-500: full index **+0.211 Sharpe / +4.21pp CAGR**, gated pool **+0.134 / +2.88pp**; the external tradeable pair **SPY - RSP +0.232 / +4.19pp**. All halves and all three thirds positive; phase sd 0.001-0.03. But the Sharpe difference itself is only **t = 1.49/1.52 (Memmel/block-bootstrap, MC sd 0.024)**, gated **0.99/1.05**, SPY-RSP **1.74/1.72** — below even the confirmatory bar of 2. Ten years cannot resolve a 0.23 Sharpe gap between two books correlated 0.92. S&P 1500 (survivorship-biased) full index only **+0.049 (t 0.32)** because equal-weighting today's mid/small survivors is a survivorship bet; its gated pool still gives **+0.129**. |
| H31b | 2026-08-09 | The construction gap is LARGER than the selection layer's +0.030 Sharpe. | gap > 0.030 | **CONFIRMED — by 4.5x to 7.7x.** +0.134 (gated, PIT-500) to +0.232 (SPY-RSP) against +0.030. Round 4's diagnosis is the right one: construction, not selection, is where the deficit lives. |
| H31c | 2026-08-09 | WEIGHTING SWEEP on the SAME top-N momentum picks: Sharpe is monotone in the cap exponent p (equal 0, sqrt 0.5, diversity 0.76, cap 1). | strictly increasing in p | **CONFIRMED, 7 of 8 size-universe cells strictly monotone.** PIT-500 n=10/20/30/50 all strictly increasing; S&P 1500 the same except n=10 where cap 0.718 ties div0.76 0.719. Inverse-vol weighting lands with equal weight (p ~ 0), not between. |
| H31d | 2026-08-09 | **THE QUESTION THAT MATTERS.** SOME weighting of a gated momentum book beats SPY on Sharpe after 10 bps round trip. | Sharpe(book) > Sharpe(SPY) | **REJECTED, 0 of 48 books, in BOTH universes, and NOT because of costs.** Best PIT-500 **mom10 cap 0.754 vs SPY 0.785** (t of the Sharpe difference -0.14); best S&P 1500 **mom50 cap 0.840 vs 0.862** (t -0.06). Every break-even cost is NEGATIVE — mom10 cap needs a **-6.9 bps SUBSIDY** per round trip to draw level, i.e. at ZERO cost it still loses. Deflated Sharpe of the best book at the registry's running N=729: **0.183**. |
| H31e | 2026-08-09 | CONCENTRATION. A 30-name equal-weight book has MORE effective bets than the cap-weighted index. | eff_bets(EW30) > eff_bets(cap index) | **REJECTED, and the failure is the finding.** By NAME count the premise holds spectacularly (equal-weight index 438 effective names vs the cap index's **58.1**, which reproduces SPY's published ~50-60). By Meucci's risk-entropy it inverts: EW30 **1.18** effective bets against the cap index's **2.56** and mom30 cap's **3.25**. Equal weighting spreads names and concentrates RISK on the single market factor; cap weighting concentrates names and buys a second, idiosyncratic mega-cap bet. Counting positions answers a question nobody asked. |
| H31f | 2026-08-09 | FERNHOLZ. gamma* (excess growth rate) is positive everywhere and largest for equal weight — and still not enough; cap wins, i.e. the diversity condition failed this decade. | gamma*(EW) > gamma*(cap) > 0 AND cap beats EW | **CONFIRMED, both clauses, and the magnitudes are the point.** PIT-500 index: gamma* equal **3.89%/yr** vs cap **3.49%/yr**, so the rebalancing premium favours equal weight by **+0.40pp/yr** — while cap beat equal by **+4.21pp/yr** realised. The theorem paid, and lost by a factor of ten. S&P 1500: gamma* 5.84 vs 4.08 (+1.76pp) against a realised CAGR gap of -0.15pp — there the premium nearly covers it, which is exactly what a survivorship-tilted small-cap universe should look like. |
| H31g | 2026-08-09 | RULE 9. No conclusion depends on which of the 42 entry offsets the capital started on. | phase sd small vs the effect | **CONFIRMED for the construction claim, and it is a WARNING for the selection claim.** Index books: phase sd 0.001-0.005. Momentum books: sd 0.15-0.17, range 0.39-1.10. **24-26% of the 42 entry schedules of a cap-weighted momentum book beat SPY; 0% of the equal-weighted ones do.** Quoting one schedule would have produced "momentum at Sharpe 1.10 beats SPY" — the Round-2 error verbatim. Pooled, it loses. |
| H31h | 2026-08-09 | CONTROL, registered ex ante: momentum picks beat a RANDOM pick of the same size from the same gated pool under the SAME weighting (20 draws per cell). | z > 0 | **REJECTED — momentum is risk-adjusted NEGATIVE against its own pool.** PIT-500 z: equal n=30 **-6.27**, cap n=30 **-2.21**, and 22 of 24 cells negative; only cap n=10 (+1.35) and div0.76 n=10 (+0.39) are positive and neither is significant. Momentum's CAGR IS higher than random (+2.3 to +6.8pp) — it buys return with volatility, which is Rule 13's sentence. S&P 1500 the same shape, all 24 cells within 2.7 sd. |
| H31i | 2026-08-09 | SENSITIVITY, registered ex ante: the answer does not come from the cap-coverage hole (SEC facts are keyed to the CURRENT ticker, so acquired members lose their share counts). | small | **PASSES, and the leak is priced.** The equal-weight index book WITHOUT the cap-data requirement scores **0.581** against **0.613** with it, so the whole coverage tilt is **+0.032 Sharpe / +0.55pp CAGR** — smaller than every effect quoted, and it biases IN FAVOUR of the books that lost. Coverage is 87% of index members overall but 74% in 2017 rising to 98% in 2026; that gradient is the leak's shape. |
